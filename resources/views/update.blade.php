...
<body>
<form method="post" action="{{ route('update_post', ['id' => $record->id]) }}" enctype="multipart/form-data" class="w-full max-w-lg mx-auto">
    @csrf
    @method('patch')
    <div class="mb-4">
        <label for="firstname" class="block text-gray-700 text-sm font-bold mb-2">Car Model</label>
        <input type="text" value="{{$record->carmodel}}" name="model" id="firstname" class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
    </div>
    <div class="mb-4">
        <label for="lastname" class="block text-gray-700 text-sm font-bold mb-2">Color</label>
        <input type="text" name="color" value="{{$record->color}}" id="lastname" class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
    </div>
    <div class="mb-4">
        <label for="education_qualification" class="block text-gray-700 text-sm font-bold mb-2">Mileage</label>
        <input type="text" name="mileage" value="{{$record->mileage}}" id="education_qualification" class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
    </div>
    <div class="mb-4">
